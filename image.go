...
func get_colour(esc float64, maxIterations float64, colour_mode string) color.NRGBA {
	if colour_mode == "true" {
		var point = esc / maxIterations
		var redpoint = redInterpolant(point)
		var greenpoint = greenInterpolant(point)
		var bluepoint = blueInterpolant(point)
...
func Draw_Image(filename string, plot_map map[Key]CalculatedPoint, width int, height int, gradient string, maxIterations float64, colour_mode string) {
